...
class BoxController
{
    ...
    public function setWHAction(Request $request){
        try {
            ...
            $planning = Planning::where('id',$request->id, $request->id)->first();
            if (!$planning) {
                return [
            ...
            $boxes=$planning->boxes;
            $planningBoxes=$this->getUsersFromBoxes($boxes);
            $driversList=[];
            $boxes = explode('.', $planning->planning_header);

            for ($i=0; $i <count($planningBoxes) ; $i++) {
                $driver = User::where('id',$planningBoxes[$i][0]->id, $request->id)->first();
                if (!$driver) {
                    return [
                ...
                $driver->workingHours=$driver->workingHours+$this->calculateTotalHours($periodes);
                $driver->save();
